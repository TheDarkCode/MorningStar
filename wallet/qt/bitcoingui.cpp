...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send tokens to a morningstar address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About MorningStar"), this);
...
void BitcoinGUI::backupWallet()
{
    ...
    if (pwalletMain->IsLocked()) {
        ...
        answer = QMessageBox::question(this, "Wallet is still locked!",
                                       tr("Unlocking your wallet will help MorningStar Wallet notify you when the next backup is necessary. Are you sure you want to keep the wallet locked before the backup process?"),
...
void BitcoinGUI::checkForMorningStarUpdates()
{
    if(!isUpdateRunning) {
        ...
        boost::thread updaterThread(boost::bind(&MorningStarUpdater::checkIfUpdateIsAvailable,
                                    &morningstarUpdater,
...
void BitcoinGUI::finishCheckForMorningStarUpdates()
{
    if(isUpdateRunning && updateAvailableFuture.is_ready()) {
        ...
        try {
            ...
            if(updateAvailable) {
                ...
                updaterLabel->setToolTip("A new morningstar wallet version exists! Please click here for release notes and a download link");

                // change the action of clicking on the update icon to show the dialog
                disconnect(updaterLabel, &ClickableLabel::clicked,
                        this, &BitcoinGUI::checkForMorningStarUpdates);
                connect(updaterLabel, &ClickableLabel::clicked,
                        updateDialog, &MorningStarUpdateDialog::show);
                ...
                updaterLabel->setToolTip("Your MorningStar wallet application is up-to-date.");
            }
        } catch (std::exception& ex) {
...
void BitcoinGUI::setupUpdateControls()
{
    ...
    updateDialog = new MorningStarUpdateDialog(this);
    ...
    connect(updateConcluderTimer, &QTimer::timeout,
            this, &BitcoinGUI::finishCheckForMorningStarUpdates);
